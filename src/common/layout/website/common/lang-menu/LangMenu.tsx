...
//       <div
//         className={`absolute md:top-full md:left-0 md:w-[140px] w-full flex flex-col bg-white border rounded-md z-[120000] shadow-md
...
// src/layout/common/lang-menu/LangMenu.tsx
import { useState, useMemo, useCallback, useEffect } from "react";
import { useTranslation } from "react-i18next";
import { useLanguage } from "../../../../../store/LanguageProvider";
import { IoIosArrowDown } from "react-icons/io";
...
const LanguageOption = ({
  ...
  return (
    <button
      onClick={() => onClick(lang.label)}
      className="flex items-center justify-between gap-2 px-3 py-2 hover:bg-gray-100 focus:outline-none focus:bg-gray-200 text-sm"
      role="menuitem"
    >
      <span>{t(lang.title)}</span>
      <img
...
const LanguageDropdown = () => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="transition duration-200 group-hover:text-primaryGreen text-xs md:text-sm">
          {t(currentLang.title)}
        </span>
        <img
          alt={currentLang.title}
      ...
      <div
        className={`
          absolute md:top-full text-start ${
            language === "en" ? "md:right-0" : "md:left-0"
          } md:w-[140px] w-full
          flex flex-col bg-white border rounded-md z-[120000] shadow-md
